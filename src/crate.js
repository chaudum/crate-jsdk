...
;(function(){
  ...
  var win = window,
      doc = document,
      $ = reqwest,
      __arg = function __arg(val, def) { return (val === null || typeof val === 'undefined') ? def : val; };
  ...
  Cluster.prototype.fetch = function fetch(){
    ...
    var fetchInfo = function(approve, reject){
      var c = self.client;
      c.execute('select id, name, master_node, settings from sys.cluster')
      .then(function(data){
        var cluster = data.toObjectArray()[0];
        for (var key in cluster) self[key] = cluster[key];
        var count = 0;
        // table information
  ...
  Cluster.prototype.update = function update(){
    ...
    if (t.length && s.length) {
      ...
      var health = this.tables.reduce(function(memo, table, idx){
        return Math.max(memo, table.health);
      }, -1);
  ...
  Cluster.prototype.setHealth = function setHealth(code){
    this.health = {
      'code': code,
      'name': (code > -1) ? ['good','warning','critical'][code] : 'unknown'
    };
  };
   ...
   * Table Information
   *
   * This JS class is private,
   * because it is only used for internal cluster/table health calculation.
   */
  ...
  Query = function(host){
    this.sql = '/_sql';
    this.host = __arg(host, DEFAULT_HOST);
  };

  Query.prototype.getURI = function getURI(){
    var h = this.host + this.sql;
    if (!h.match('^http')) {
      h = 'http://' + h;
    }
    return h;
  ...
  Query.prototype.execute = function(stmt, a, b){
    ...
    return new Promise(function(resolve, reject){
      $(options).then(function(d){
        var data = clone(d);
        data.request = body;
        var response = new SQLResponse(data)
	resolve(response);
      }).catch(function(xhr){
